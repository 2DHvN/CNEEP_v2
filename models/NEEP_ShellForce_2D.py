...
class _ShellForceBranch2D(nn.Module):
    ...
    def __init__(
        ...
        self.n_force_components = (
            n_components if n_force_components is None else int(n_force_components)
        )
        if self.n_force_components <= 0:
            raise ValueError("n_force_components must be positive.")
...
class MultiScaleShellForceCNEEP2D(nn.Module):
    ...
    The model decomposes entropy production into shell-wise generalized work:
        x_mid = 0.5 * (x_t + x_{t+dt})
        dq    = selected state increments plus optional external increments
        dS_k  = sum_i F_k(x_mid)_i dot dq_i

    This makes every branch antisymmetric under time reversal by construction
    when every external increment is itself time-odd.  If the caller reverses
    ``x``, it must also negate ``external_increments``.  J_k then has the
    interpretation of the EP increment generated by the learned k-shell force.
    """
    ...
    def __init__(self, opt):
        ...
        # Some input fields are useful context for the learned force without
        # being generalized coordinates in the final force·increment product.
        # Keep the historical all-component contraction when this option is
        # absent, while allowing angle notebooks to select (Δrho, δtheta).
    ...
    def _split_short_time_pair(self, x: torch.Tensor):
        if self.n_components > 1:
            ...
            if x.shape[2] != self.n_components:
                raise ValueError(
                    f"Expected {self.n_components} state components, got {x.shape[2]}."
                )
            x0 = x[:, 0]
            x1 = x[:, 1]
        else:
        ...
        x_mid = 0.5 * (x0 + x1)
        dx = x1[:, self.ep_component_indices] - x0[:, self.ep_component_indices]
        return x_mid, dx
    ...
    def forward(
        ...
        x:
            ...
            Every channel conditions the force, while ``ep_component_indices``
            selects which channel increments enter the final contraction.
        return_maps:
            If True, return [B, K, Lx, Ly] local EP maps.
            If False, return [B, K] shell EP increments averaged over space.
        return_forces:
            If True, return learned force maps [B, K, C_force, Lx, Ly]. If both
            return_maps and return_forces are True, returns (maps, forces).
            Force channels follow ``ep_component_indices`` first, then the
            configured external-increment order.
        external_increments:
            Generalized increments [B, C_external, Lx, Ly], such as a
            particle-resolved wrapped delta-theta map.  This argument is
            required when ``n_external_ep_components > 0``.  The increments
            enter only the final force contraction and are not state/context
            channels.  They must be negated whenever the two frames are swapped.
        """
