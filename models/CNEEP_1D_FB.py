...
class CNEEP(nn.Module):
    def __init__(self, opt):
        ...
        tmp = nn.Sequential()
        if opt.periodic:
            tmp.add_module("periodic_pad1", PeriodicPad1d(padding=2))
        else:
            tmp.add_module("pad1", nn.ConstantPad1d(padding=2, value=0))
        tmp.add_module("conv",
        ...
        self.decoder = nn.Sequential(
            nn.Conv1d(bottleneck_channels, opt.n_channel, kernel_size=1),
            nn.Upsample(scale_factor=upsample_factor, mode='linear', align_corners=True),
            nn.Conv1d(opt.n_channel, 1, kernel_size=1),
        )
